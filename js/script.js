...
async function askBot(IAName) {
  ...
  if (!answer.ok) {
    console.error(`Error while fetching (${answer.status})`);
    return false;
  }
...
function addMessageIa(message, li, iaName) {
  li.innerHTML =
    `<div class=\"iaSender sender\">${iaName}: </div>` +
    mdConverter.makeHtml(message);

  speak(li);
  if (message) oldMessages.push({ role: "assistant", content: message });
  hljs.highlightAll();

  // Scroll down to see all the message
  fieldset.scrollTo(0, ul.scrollHeight);
}
...
document.addEventListener("DOMContentLoaded", () => {
  if (localStorage.getItem("messages")) {
    ...
    oldMessages.push({
      role: "user",
      content: "You have to talk in the language : " + navigator.language,
    });
  }
